...
import { toast } from 'sonner'; // Или другая библиотека, которую ты используешь
import { passwordApi } from '../api/index.js'; // Укажи свой путь до файла
...
export default function ResetPasswordScreen({ onBackToLogin }: Props) {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err.response?.data?.error || 'Ошибка при отправке кода';
      toast.error(message);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onBackToLogin(); // Возвращаем юзера на логин
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-blue-50 dark:bg-gray-900 flex items-center justify-center p-4 transition-colors">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl w-full max-w-md border-t-4 border-blue-600">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm text-center mb-6">
          {step === 1 
            ? 'Введите номер, чтобы получить код подтверждения' 
